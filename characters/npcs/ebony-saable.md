# Ebony Saable

## Document Status

- Type: NPC dossier
- Maturity: Draft, created from source extract and GM identification; updated with survivor-set confirmation
- Scope: Man's Promise rigger, Sho "monk man" reference, confirmed tracked survivor

## Overview

Ebony Saable is a short, dark, bald man with lean muscle and a habit of silence.

He is tagged as a **Rigger** associated with **The Man's Promise**.

Current GM identification resolves him as the "monk man" referenced by Sho.

Ebony is one of the six tracked Man's Promise survivors.

## Identity

- Name: Ebony Saable
- Type: NPC
- Role: Rigger
- Listed location / tag: The Man's Promise
- Current location: Man's Promise survivor context; exact physical position pending confirmation
- Current status: Alive / tracked Man's Promise survivor
- Survivor set: One of the six tracked Man's Promise survivors

## Appearance and Bearing

Ebony is a dark, short, bald man, rippled with lean muscles.

He is most often silent.

## Survivor Set

Ebony is one of the six tracked Man's Promise survivors:

- Dahrehn Numus
- Ebony Saable
- Goyle Onórach
- Kahina Bukekayo
- Merchiel V'giatore
- Nor'gal Daraa

## Campaign Role

Ebony helps fill out the named Man's Promise survivor bloc around Sho's current prisoner / hot-box thread.

Because Sho carries guilt over the Man's Promise crew and because some survivors may be preparing to let her out to help with mutiny, named survivor identities matter.

Ebony should be treated as a possible witness, obstacle, or quiet participant in that survivor bloc rather than as a generic background sailor.

## Sho Reference

Current GM identification: Ebony Saable is the "monk man" Sho referenced.

The exact content of Sho's reference still needs to be tied back to the recovered Sho notes or logs.

Do not infer more than the identification and survivor-set status: Ebony's actual attitude toward Sho, whether he trusts her, and whether he supports freeing her remain unresolved.

## Current State

- Location: Man's Promise survivor context; exact physical position pending confirmation
- Status: Alive / tracked survivor
- Faction position: Man's Promise survivor bloc, stance unresolved
- Practical role: Rigger / physically capable sailor
- Immediate relevance: Named survivor near Sho's current guilt and mutiny thread

## Relationships

### Sho Astor

Ebony is now identified as the "monk man" Sho referenced.

His exact relationship with Sho is unresolved.

He may matter if Sho's attempts to make amends with Man's Promise survivors involved him, or if he witnessed the boarding, surrender promises, or aftermath.

### Kahina Bukekayo

Ebony belongs to the same Man's Promise survivor set as Kahina, but his relationship with her is unresolved.

### Goyle Onórach

Goyle was formerly tied to the Man's Promise crew structure. Ebony's relationship with Goyle is unresolved.

### Plugg and Scourge

Ebony's stance toward Plugg and Scourge is unresolved.

As a confirmed Man's Promise survivor, he may oppose them, submit for survival, or wait to see who can actually hold the ship.

## Campaign Function at Relaunch

Ebony can serve as a quiet, physically credible Man's Promise survivor whose opinion may matter during the hot-box / mutiny choice.

He should not be treated as automatically friendly to the PCs merely because he is not part of the Wormwood hostile bloc.

His rigger role may matter if the post-mutiny question becomes whether the remaining crew can actually sail the Man's Promise.

## Open Questions

- What exact Sho note or log called him the "monk man"?
- Is Ebony physically aboard the Man's Promise at the current relaunch point?
- Did Ebony witness Sho's attempt to spare surrendering sailors during the boarding?
- Did Ebony witness Rak or Harvus killing people Sho had promised safety?
- Does Ebony support Kahina's life-for-life test?
- Would Ebony support freeing Sho to oppose Plugg?
- Can Ebony help sail or rig the Man's Promise after the mutiny?
