# Wormwood Standard Day Routine

## Document Status

- Type: Evidence / AP scaffold extraction
- Maturity: Structured extract
- Source: GM-provided AP routine text and GM reconstruction notes
- Scope: Standard Wormwood days before the capture of the Man's Promise
- Canon status: Published/AP baseline scaffold unless table-specific events contradict it

## Standard Day Structure

Each standard day aboard the Wormwood before the Man's Promise fight follows a routine pattern.

## Work

The PCs spend the day at their assigned jobs.

This routine becomes the baseline pressure of the Wormwood: labor, fatigue, job checks, supervision, and the practical reality that the ship owns the characters' daylight hours.

## Bloody Hour

At dusk, the crew gathers on deck to witness punishments.

On the first night, this is demonstrated by the keelhauling of **Jakes Magpie**, a pirate caught stealing from the quartermaster's store the night before.

After a brief interrogation by **Mr. Plugg** and **Master Scourge**, Jakes admits his crime. He is brought up from the bilges to be slowly keelhauled before the evening meal.

The players notice the relish with which Plugg, in particular, tugs at the rope as Magpie is dragged under the ship.

What appears on the other side has been cut to shreds and is soon thrown overboard to feed the sharks.

## Leisure

After dinner and the rum ration, various pirate entertainments take place on deck or below for as long as people are sober enough to participate.

This is the space where influence, gambling, music, information, intimidation, and theft/recovery attempts happen.

## Rum Ration Character Note

Sho Astor never drank her rum ration even once.

We do not have direct evidence of how she got rid of it each night, but the current reconstruction is that she consistently disposed of or avoided consuming it.

Do not write Sho as willingly drinking the Wormwood rum unless later evidence directly contradicts this.

## Rest

The AP routine assumes that characters who drink their rum ration can retire to their hammocks in the lower hold whenever they wish.

For Sho, treat this as a hidden avoidance pattern rather than actual drinking.

If the PCs have the energy, they can take additional nighttime ship actions.

## Reconstruction Notes

- Day 1 should include Plugg's tests, job assignments, the first work period, Jakes Magpie's keelhauling, dinner/rum/leisure, Sandara returning Mirei's harp, and sleep.
- Most routine days before the Man's Promise fight likely follow the AP pattern with table-specific night actions layered on top.
- Some nuance may be lost unless recovered from player comments, chat logs, or old prose.
- Avoid inventing detailed night actions where the log is missing.
- Preserve Sho's rum-ration avoidance as a recurring character truth, while leaving the method unresolved.

## Open Checks

- Confirm which standard days have recoverable table-specific scenes.
- Confirm whether the first night's Sandara/Mirei/Harvus scene occurred before or after rum ration.
- Confirm whether any first-night action besides the Sandara harp scene survived in player memory or logs.
- Confirm whether Sho disposed of, traded, hid, poured out, or otherwise avoided her rum ration.
