# Recovered Tactical Map — Man's Promise Fight
...
The map confirms the following movement and battle shape:

- The boarding party secured the aft deck and the ship's wheel during the fog-bound fighting.
- From the aft deck, the PCs worked their way down toward the middle deck.
- The central fight concentrated between the two lifeboats and near the main mast.
- This aligns with the existing reconstruction that Sho secured or held the wheel, tried to impose order, and the fight then became a confused central-deck struggle.

## Confirmed Death / Position Evidence

The map confirms that **Giffer Tibbs died below the stairs on the port side**.

This clarifies the previous uncertainty around whether Giffer died immediately or later: the tactical map places her death during the Man's Promise fight itself, below the port-side stairs.
...
A Man's Promise crew member was in or around that boat when Rak triggered or released it too early.

The crew member ended up in the water.
...
The map also confirms surrender states and injured Wormwood crew positions, but this note does not yet identify every token by name.

Use the screenshot as a strong prompt for token-by-token reconstruction, but do not assign specific named surrender or injury outcomes beyond the current confirmed notes without a token key or additional GM confirmation.

## Canon Impact

This map evidence upgrades the following from uncertain or hearsay to strong reconstruction truth:

- PC tactical path: aft deck / wheel first, then central deck by lifeboats and main mast.
