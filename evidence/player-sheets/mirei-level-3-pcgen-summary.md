# Mirei Level 3 PCGen Sheet Summary

## Document Status

- Type: Player sheet evidence summary
- Source: Uploaded PDF `Mirei (1).pdf`
- Tool / date on sheet: PCGen v6.04.01, created Aug 2, 2015 at 6:31:45 PM
- Character sheet level: Level 3, CR 2
- Canon status: Mechanical evidence. Use as the strongest recovered Mirei mechanics anchor unless a later sheet or player correction supersedes it.
- Caution: The sheet contains obvious display/export anomalies in physical statistics, including height rendered as `0' 5" / 140 lbs.` Do not overwrite established visual dossier values from that field without player confirmation.

## Identity Snapshot

- Character: Mirei
- Deity: Desna
- Region: None on this sheet
- Alignment: Chaotic Good
- Class: Bard (Sea Singer) 3
- Race: Aasimar (Azata-Blooded) / Outsider
- Size: Medium / 5 ft. x 5 ft.
- Vision: Darkvision 60 ft.
- Age: 65
- Gender: Female
- XP: 5000 / 9000
- Money: 0 gp
- Languages: Celestial, Common, Elven, Sylvan

## Core Combat / Defense Snapshot

- HP: 25
- Speed: Walk 30 ft.
- AC: 14 total; 10 flat-footed; 14 touch
- Initiative: +4
- Fortitude: +3
- Reflex: +7
- Will: +3
- Base attack bonus: +2
- Melee attack bonus: +2
- Ranged attack bonus: +6
- CMB: +2
- CMD: 18
- Encumbrance: Light

## Ability Scores

- Strength 10 (+0)
- Dexterity 18 (+4)
- Constitution 14 (+2)
- Intelligence 15 (+2)
- Wisdom 11 (+0)
- Charisma 18 (+4)

## Skills

The sheet lists 24 total skill points and max ranks 3 / 3.

Notable trained or emphasized skills:

- Acrobatics +12
- Diplomacy +10
- Perform (Dance) +10
- Perform (Sing) +12
- Perform (String Instruments) +13
- Profession (Sailor) +7
- Perception +8
- Sense Motive +6
- Swim +7
- Knowledge (Local) +7
- Bluff +8
- Climb +6
- Escape Artist +8
- Stealth +8

## Traits

- Lillend's Harp
- Touched by the Sea

## Feats

- Nimble Moves
- Prodigy (Perform (Sing), Profession (Sailor))
- Alertness (Granted)

## Sea Singer / Bard Abilities and Qualities

- Bardic Performance: 12 rounds per day
- Distraction
- Fascinate, DC 15
- Inspire Courage +1
- Sea Shanty
- Sea Legs
- Still Water
- World Traveler
- Familiar: Sea Singer exotic pet at 2nd level, monkey or parrot treated as raven; the sheet does not identify the familiar's species or name.
- Armored Casting
- Cantrips
- Celestial Resistance: acid 5, cold 5, electricity 5
- Skilled: Azata-blooded aasimar +2 Diplomacy and Perform checks
- Spell-like ability: Glitterdust 1/day

## Spells

### Innate Racial Spell

- Glitterdust 1/day

### Bard Spellcasting

- Concentration +7
- Level 0 known: 6
- Level 1 known: 4
- Level 1 spells per day: 4

### Bard Cantrips Known

- Detect Magic
- Light
- Mage Hand
- Mending
- Message
- Summon Instrument

### Bard Level 1 Spells Known

- Charm Person
- Chord of Shards
- Cure Light Wounds
- Hypnotism

## Proficiencies

The sheet lists broad bard / campaign weapon proficiencies, including but not limited to:

- Dagger
- Light crossbow
- Rapier
- Shortbow
- Short sword
- Whip
- Longsword
- Sap
- Spear
- Ray and touch spells
- Splash weapon

## Integration Notes

- Update `characters/pcs/mirei.md` so the level 1 sheet is no longer the only recovered mechanical anchor.
- Replace the old statement that Mirei is currently level 2 with a cautionary statement that a level 3 sheet has been recovered and should be verified as the relaunch-current or latest pre-relaunch mechanics.
- Preserve established dossier visual details unless the player confirms the level 3 sheet's physical-stat fields are intended; the PDF's height field appears malformed.
- For PF2e conversion, the new mechanical identity pillars are: Bard 3, Sea Singer, Still Water, Sea Legs, World Traveler, Familiar, Chord of Shards, Cure Light Wounds, Prodigy in Perform (Sing) and Profession (Sailor), high Dexterity / Charisma, and strong Acrobatics / Performance / social / sailor skill emphasis.
