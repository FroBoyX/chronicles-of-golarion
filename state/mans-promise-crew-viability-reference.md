# Man's Promise Crew Viability Reference

## Document Status

- Type: Current-state reference / correction note
- Maturity: Working control document, corrected with Badger / Jack aboard, Maheem / Kipper / Patch believed Wormwood-side, and locked Rahadoumi-officer example sequence
- Scope: Erastus 27 ship split, Arodus 5 Man's Promise actor pool, skeleton-crew viability, and ship crew requirement reference
- Canon status: Current GM truth plus rules-reference note unless superseded by later direct evidence

## Purpose

Use this file as the compact reference for the Man's Promise / Bonewrack crew-state problem.

This file corrects the older drift where the eight unnamed Man's Promise survivors were left in an unresolved Arodus 5 actor bucket. It also clarifies the ship-crew requirement used when deciding whether the Man's Promise can be sailed after a mutiny or faction split.

## Rules Reference — Sailing Ship Crew Requirement

The Man's Promise should be treated as a Pathfinder First Edition sailing ship for crew-viability purposes.

Working reference:

- Pathfinder First Edition sailing ship vehicle statistics list **Crew 20**.
- In this archive, **20 crew** means the effective operating crew needed to sail the ship properly and sustainably.
- This does **not** mean the ship cannot be moved at all with fewer than twenty people in an emergency, but below that level the ship should be treated as under-crewed, slow, unsafe, unable to maintain watches properly, and vulnerable to failed repairs, bad weather, combat, fatigue, and morale collapse.

Campaign application:

- A post-mutiny victory does not automatically mean the PCs can sail away.
- The ship needs enough willing or controllable people to cover rigging, swab work, repairs, bilge/water control, helm/watch, cooking, and command acceptance.
- The current crew-state question is therefore not just **who wins the fight**, but **who remains alive, aboard, capable, and willing to work afterward**.

## Corrected Erastus 27 Ship-Split Model

The ship split on **Erastus 27, 4712 AR**.

Correct sequence:

1. The Wormwood captured the Man's Promise on Erastus 25.
2. The post-battle captured Man's Promise survivor lineup contained **15 captured survivors**.
3. Captain Harrigan made an example of the **only surviving Rahadoumi officer** by throwing that officer overboard.
4. **14 captured Man's Promise survivors** remained alive after that execution.
5. Harrigan assigned Mr. Plugg a skeleton crew and Plugg claimed captaincy of the Man's Promise.
6. The Wormwood and the Man's Promise separated on Erastus 27.
7. Of the fourteen surviving captured Man's Promise crew, **six named survivors** were assigned to Plugg's skeleton crew.
8. The other **eight surviving captured Man's Promise crew went with the Wormwood**.

Older notes that left a Rahadoumi officer visibly alive after the Erastus 27 example are superseded unless later evidence identifies a separate non-officer witness or another distinct survivor.

## Six Named Man's Promise Survivors Assigned to Plugg

These six are the fixed named survivor bloc assigned to the Man's Promise under Plugg:

| Character | Role | Arodus 5 state |
|---|---|---|
| Dahrehn Numus | Swab | Likely Man's Promise; exact physical position unresolved. |
| Ebony Saable | Rigger | Likely Man's Promise; confirmed as Fipps' `dark one` / Sho's `monk man` thread; exact cold-open position unresolved. |
| Goyle Onórach | Swab / former quartermaster context | Bonewrack, with Harvus/Mirei/Sandara/Tilly after rescue. |
| Kahina Bukekayo | Rigger | Man's Promise, active around Sho's hot-box / life-for-life test. |
| Merchiel V'giatore | Rigger | Likely Man's Promise; exact physical position unresolved. |
| Nor'gal Daraa | Swab | Likely Man's Promise; revenge thread unresolved. |

Do not conflate these six with the full survivor count. They are the named survivors assigned to Plugg, not the only people who survived the battle.

## Other Eight Man's Promise Survivors

The other eight surviving captured Man's Promise crew are **not** Arodus 5 Man's Promise / Bonewrack actors.

Correct handling:

- They went with the Wormwood after the Erastus 27 ship split.
- Their names and later Wormwood-side fates remain unresolved.
- They should not be counted toward the Man's Promise crew available at Bonewrack.
- They should not be used as unnamed relaunch actors unless later evidence brings them back.
- Under current reconstruction, they do not include the only surviving Rahadoumi officer thrown overboard by Harrigan.

## Current Arodus 5 Actor Pool

The Arodus 5 Man's Promise / Bonewrack pool consists of:

1. PCs transferred to the Man's Promise.
2. Wormwood crew transferred to Plugg's skeleton crew.
3. The six named Man's Promise survivors assigned to Plugg.
4. Any later confirmed Bonewrack / Man's Promise actors shown in post-Erastus 27 chat evidence.

It does **not** include the other eight captured Man's Promise survivors, because they are Wormwood-side after the split.

## Evidence-Backed Wormwood Transfers / Man's Promise-Side Crew

| Character | Evidence status | Arodus 5 handling |
|---|---|---|
| Mr. Plugg | Locked transfer / captain claim | Man's Promise command threat. |
| Master Scourge | Locked transfer / first mate | Man's Promise command threat. |
| Slippery Syl Lonegan | Locked transfer / boatswain | Bonewrack, Rak group. |
| Ambrose Kroop | Locked transfer / cook | Likely aboard Man's Promise unless later evidence places him elsewhere. |
| Fipps Chumlett | Locked post-split evidence | Coconut Beach rendezvous actor. |
| Owlbear Hartshorn | Locked post-split evidence as Plugg's cabin / door guard | Likely aboard Man's Promise; exact cold-open position unresolved. |
| Crimson Cogward | Locked post-split evidence aboard | Exact Arodus 5 position unresolved. |
| Ratline Rattsberger | Locked post-split evidence aboard | Exact Arodus 5 position unresolved. |
| Sandara Quinn | Locked aboard before abduction | Bonewrack, Harvus group after rescue. |
| Tilly Brackett | Locked aboard before abduction | Bonewrack, Harvus group after rescue. |
| Jaundiced Jape | Locked post-split evidence aboard | Bonewrack, Rak group. |
| Tam "Narwhal" Tate | Locked post-split evidence aboard | Exact Arodus 5 position unresolved. |
| Shivikah | Strong post-split evidence aboard | Bonewrack, Rak group. |
| Barefoot Samms Toppin | Strong course-change warning evidence; placement still technically pending raw-source reconciliation | Likely Man's Promise / movable survival bloc. |
| Badger Medlar | GM-corrected aboard Man's Promise | Exact Arodus 5 position unresolved. |
| Jack Scrimshaw | GM-corrected aboard Man's Promise | Exact Arodus 5 position unresolved. |

## Confirmed / Believed Wormwood-Side / Not With Man's Promise

| Character | Status |
|---|---|
| Cut-Throat Grok | Confirmed remained aboard the Wormwood when Rak left. |
| Maheem | Believed Wormwood-side. |
| Kipper | Believed Wormwood-side. |
| Patch Patchsalt | Believed Wormwood-side. |
| Other eight surviving captured Man's Promise crew | Went with the Wormwood after the Erastus 27 split. |

## Still Unresolved for Skeleton-Crew Viability

- Conchobhar Turlach Shortstone placement.
- Exact Arodus 5 positions for Crimson, Ratline, Tam, Samms, Badger, Jack, Dahrehn, Ebony, Merchiel, and Nor'gal.
- Whether Owlbear is chained, loose, guarding, or otherwise positioned under Plugg.
- Who can still rig, repair, cook, pump, steer, stand watch, and accept command after a mutiny.

## Count Snapshot

| Count | Value |
|---|---:|
| Locked / strong Wormwood-origin Man's Promise transfers | 20 |
| Six named captured Man's Promise survivors assigned to Plugg | +6 |
| Man's Promise storm-onset count, current working count | **26** |
| Add Conchobhar if confirmed Man's Promise-side | +1 |
| Man's Promise storm-onset count with Conchobhar | **27** |
| Wormwood after split, current working count | **18** |
| Wormwood after split if Conchobhar is Man's Promise-side | **17** |
