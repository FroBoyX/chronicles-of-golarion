# Sho Astor Visual Brief

## Document Status

- Type: Character visual brief
- Maturity: Draft
- Subject: Sho Astor
- Asset type: Portrait
- Spoiler scope: Player-safe if cropped/context-controlled
- Canon status: Working visual brief, no approved visual canon yet
- Proposed asset ID: `sho-hotbox-portrait-v1`
- Anchor asset: Intended first character portrait anchor if approved

## Source Basis

- Canon/dossier files:
  - `characters/pcs/sho-astor.md`
  - `design/visual-style-guide.md`
- Evidence files:
  - PCGen-derived identity and state evidence summarized in Sho dossier
  - Player-memory answers summarized in Sho dossier
- Map notes/AP scaffold:
  - `book-1/mans-promise.md` for current Man's Promise / hot-box state
- GM corrections:
  - Current relaunch state: Sho is aboard the Man's Promise, imprisoned in the hot-box after being beaten unconscious by Scourge's shore group.
  - Kahina Bukekayo has spoken with Sho and challenged whether Sho's remorse has action behind it.
- Existing visual anchors:
  - None.
- Reference notes:
  - Use Pathfinder/Golarion/Skull & Shackles pirate-fantasy tone for costume language, shipboard grime, and punishment context.
  - Use reference material only for material culture, mood, palette, and silhouette family; do not directly reproduce Paizo art.

## Known Visual Facts

- Sho Astor is a female suli monk from Jalmeray.
- Age: 24.
- Skin: soft lavender.
- Eyes: piercing solid blue.
- Hair: silver, short.
- Presentation: often masculine / gender-neutral, rooted in monastery habit, safety, and personal boundary.
- Clothing logic: functional, restrained, monk-practical, not decorative unless explicitly tied to Agatha or later state change.
- Current state for this asset: imprisoned in the hot-box aboard the Man's Promise, physically punished and under moral pressure.
- Sho's cloak is a selfhood anchor connected to Agatha Dreamriver, but she likely hides it rather than wearing it openly in the hot-box scene.

## Unresolved Visual Conflicts

- Height conflict: PCGen export suggests 5'5" / 65 inches, while Harvus' light-log describes Sho as 6'2", wiry, short-haired, and pale-purple-skinned. Do not emphasize full height in this portrait until resolved.
- Exact hot-box construction has not been visually locked. Treat it as a cramped shipboard punishment box, but avoid over-defining tactical construction.
- Exact current wound pattern after Scourge's group beat her unconscious remains open. Show bruising, dehydration, heat stress, and confinement strain without inventing named scars.
- Irori faith is sheet-supported but not confirmed as active religious identity. Do not add overt holy symbols unless later confirmed.

## Required Visual Facts

- Soft lavender skin, short silver hair, solid piercing blue eyes.
- Disciplined restraint even under captivity.
- Hot, cramped, oppressive shipboard punishment context.
- Gender-neutral / masculine practical presentation rather than ornate feminine styling.
- Salt, sweat, bruising, dried blood or grime may be present if subtle and state-appropriate.
- Expression should carry moral pressure: anger restrained into decision, not generic helplessness.

## Character Identity Cues

- Suli / Jalmeray visual cues should be quiet and grounded: disciplined posture, restrained monk clothing, hints of elemental heritage through skin/eye coloration rather than flashy elemental effects.
- Strength under restraint is central.
- Sho should look like someone who has not surrendered internally, even while physically confined.
- Agatha-linked softness should be latent, not foregrounded, unless a later variant shows the cloak or a private moment.

## Style Direction

- Grimy swashbuckling fantasy.
- Hard tropical heat, stale air, salt-stained wood, rope, rusted fittings, and punished flesh.
- Pathfinder/Golarion pirate-fantasy tone, not generic medieval prison fantasy.
- Portrait should prioritize recognizable identity and story state over beauty.

## Output Requirements

- Orientation: Vertical portrait.
- Framing: Head-and-shoulders or tight half-body through/inside the hot-box, avoiding full-height commitment.
- Background: Close wooden punishment-box interior or slatted opening with blurred Man's Promise deck light beyond.
- Player-safe scope: Safe if presented as Sho's current captivity state; avoid GM-only surrounding mutiny details.
- Variants needed:
  - Later: full-body anchor after portrait approval.
  - Later: cloak / Agatha selfhood variant.
  - Later: post-hot-box escape or mutiny-state variant.

## Avoid / Do Not Drift

- Do not make Sho generic human; preserve suli coloration.
- Do not over-sexualize confinement or punishment.
- Do not make the scene clean, theatrical, or noble-court polished.
- Do not use direct Paizo art reproduction.
- Do not resolve height through full-body scale yet.
- Do not show overt Irori iconography unless GM confirms active visual faith.
- Do not place the cloak openly unless GM confirms she has access to it in this scene.

## Prompt Draft

```text
Vertical character portrait of Sho Astor, a female suli monk from Jalmeray imprisoned in a cramped shipboard hot-box aboard a captured pirate prize ship. She has soft lavender skin, short silver hair, and piercing solid blue eyes. Her clothing is practical, gender-neutral, salt-stained monk travel wear with restrained Jalmeray influence, worn and dirtied by shipboard punishment. Frame her tightly through the narrow slats or cramped opening of the hot-box so her exact height is not established. Show tropical heat, sweat, bruising, rope marks, weathered dark wood, rusted fittings, and hard sunlight cutting through the box. Her expression is controlled and furious beneath discipline: not defeated, but deciding what remorse demands. Grimy Golarion / Skull & Shackles pirate-fantasy tone, weathered materials, oppressive shipboard mood, recognizable character identity, painterly realism, no direct reproduction of official art.
```

## Asset Index Links

- Tracking row: `sho-hotbox-portrait-v1` in `tracking/image-output-index.md`
- Generated asset paths or links: None yet.

## Open Checks

- Resolve Sho height conflict before full-body output.
- Confirm exact current injuries if the portrait should show more than heat stress / bruising / confinement marks.
- Confirm whether Sho's cloak is physically available for a later variant.
- Confirm whether active Irori symbolism should ever appear visually.
